...
```{r history}
Version <- c("01", "02")
Changes <- c("Initial version", "Added detectable effect for a given sample size")
...
After the cleaning process the raw data will include `r Nvar_final` variables.
Table 1 shows the structure of the raw dataset.
...
```{r}
n.small <- pwr::pwr.t.test(d = .2, power = .8, sig.level = .05)$n %>% ceiling()
n.medium <- pwr::pwr.t.test(d = .5, power = .8, sig.level = .05)$n %>% ceiling()
n.large <- pwr::pwr.t.test(d = .8, power = .8, sig.level = .05)$n %>% ceiling()

n_max <- 5
d_10 <- pwr::pwr.t.test(n = n_max, power = .8, sig.level = .05)$d %>% philsfmisc::format.float(digits = 2)
```
...
With sample size of `r n.large` per group a t test would be able to detect an effect size of d = 0.8 with 80% power at a 5% significance level (alpha).
For this effect size the total study size would require the inclusion of
`r 2*n.large` participants.

```{r fig1, fig.cap="**Figure 1** Sample size required to detect a large effect size."}
# ![Age](../figures/outcome.png)
knitr::include_graphics("../figures/large.png", error = FALSE)
```

### Medium effect size

With sample size of `r n.medium` per group a t test would be able to detect an effect size of d = 0.5 with 80% power at a 5% significance level (alpha).
For this effect size the total study size would require the inclusion of
`r 2*n.medium` participants.

```{r fig2, fig.cap="**Figure 2** Sample size for a medium effect size."}
# ![Age](../figures/outcome.png)
knitr::include_graphics("../figures/medium.png", error = FALSE)
```

### Small effect size

With sample size of `r n.small` per group a t test would be able to detect an effect size of d = 0.2 with 80% power at a 5% significance level (alpha).
For this effect size the total study size would require the inclusion of
`r 2*n.small` participants.

```{r fig3, fig.cap="**Figure 3** Sample size required to detect a small effect size."}
# ![Age](../figures/outcome.png)
knitr::include_graphics("../figures/small.png", error = FALSE)
```

### Effect size for a given sample size

If the maximum sample size feasible is N = `r 2*n_max` (divided into two groups of `r n_max`), then the minimum effect size that could be detected by this study would be d = `r d_10`.
